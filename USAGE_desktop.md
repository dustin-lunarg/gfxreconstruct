...
Capture Specific Queue Submission | GFXRECON_CAPTURE_SUBMISSIONS | STRING | Specify one queue submission ranges to capture. A queue submission is when `vkQueueSubmission` is called and allows for a finer granularity tracing than capturing per frame. This setting **can't** be used along withGFXRECON_CAPTURE_FRAMES. A queue submission range can be specified as a single value, to specify a single queue submission to capture, or as two hyphenated values, to specify the first and last queue submission to capture. Example: `200` or `301-305`. Default is: Empty string (all queue submissions are captured).
